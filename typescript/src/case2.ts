...
const concreteMyCanvas = myCanvas as HTMLCanvasElement
...
/** 型エイリアス
 *  type 型名 = 型
 *  e.g. type Point = { x: number; y: number; }
 */
type Name = string
...
function printFormat(firstName: string, formatter: Formatter): void {
    console.log(formatter(firstName))
}


// インデックス型
// 事前にプロパティ名が決まらない場合に使用する(よく使う)
// { [] : 型名 }
// e.g. keyに文字列, valueに文字列を強制する
type Label = {
    [key: string] : string
}
const labels: Label = {
